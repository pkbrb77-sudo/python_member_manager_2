...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let editingContactId = null; // 현재 편집 중인 연락처 ID (null = 추가 모드, 값 = 수정 모드)
...
// [Enter 키 로그인 기능 추가]
                    ...
                    return `
                        <div class="contact-card" data-id="${contact.id}">
                            ...
                            <div class="card-actions">
                                <button class="edit-contact-btn" onclick="editContact(${contact.id})">수정</button>
    ...
    // ==========================================
    // 5. 회원정보 추가/수정 및 맞춤형 알림 문구 출력 (POST /contacts 또는 PATCH /contacts/{id})
    // ==========================================
    const addContactForm = document.getElementById("add-contact-form");
    const panelTitle = document.querySelector(".add-contact-panel h2");
    ...
    if (addContactForm) {
        addContactForm.addEventListener("submit", async (e) => {
            ...
            try {
                let response, data, isUpdate = false;

                if (editingContactId) {
                    // 수정 모드: PATCH 요청
                    response = await fetch(`${BACKEND_URL}/contacts/${editingContactId}`, {
                        method: "PATCH",
                        headers: { "Content-Type": "application/json" },
                ...
                if (response.status === 201 || response.status === 200) {
                    ...
                    if (panelTitle) panelTitle.textContent = "회원정보 등록";
    ...
    // ==========================================
    // 6. 회원 정보 수정 (편집 모드 활성화)
    // ==========================================
    window.editContact = async (contactId) => {
        try {
            const response = await fetch(`${BACKEND_URL}/contacts/${contactId}`, { credentials: "include" });
            if (response.ok) {
                ...
                const panelTitle = document.querySelector(".add-contact-panel h2");
                const submitBtn = document.querySelector(".submit-btn");
                if (panelTitle) panelTitle.textContent = "회원정보 수정";
                ...
                // 폼이 보이는 위치로 스크롤
                document.querySelector(".add-contact-panel").scrollIntoView({ behavior: "smooth" });
            } else {
                alert("연락처 정보를 불러올 수 없습니다.");
            }
        } catch (error) {
